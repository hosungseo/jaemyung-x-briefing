...
(function () {
  ...
  const STOP = new Set([
    ...
    "이재명","대통령","대통령님","민국","정부","나라","세계","한국","처음","다시","모두","가장","매우","정말","진심","감사",
    "가졌습니다","만났습니다","나눴습니다","되어","되어야","생각","축하","현장","시간","마음","소식","모습","자리",
    "수","것","제","저","더","또","와","과","을","를","이","가","은","는","의","에","로","도",
  ]);
  const cleanTerm = (k) => String(k || "")
    .replace(/민주주의/g, "민주주의X")
    .replace(/한반도/g, "한반도X")
    .replace(/(입니다|합니다|했습니다|드립니다|하겠습니다|이라는|라는|으로|에서|에게|까지|부터|처럼|보다|이며|이고|하고|하는|했다|한다|하게|들이|들의|으로서|으로써|께서|에게|에는|에도|만큼|조차|마저|님|은|는|이|가|을|를|의|와|과|도|로|에)$/g, "")
